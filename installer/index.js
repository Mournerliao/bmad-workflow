...
function mergeSkills({ officialSkillsDir, personalSkillsDir, destinationDir, dryRun }) {
  const officialSkills = listDirectories(officialSkillsDir);
  const personalSkills = listDirectories(personalSkillsDir);

  console.log(
    `Generating ${destinationDir} with ${officialSkills.length} official skills and ${personalSkills.length} personal skills`,
  );

  if (dryRun) {
    officialSkills.forEach((skill) => console.log(`[dry-run] official skill ${skill}`));
    personalSkills.forEach((skill) => console.log(`[dry-run] personal skill ${skill}`));
    return;
  }
  ...
  for (const skill of personalSkills) {
    copyDir(path.join(personalSkillsDir, skill), path.join(destinationDir, skill));
  }
}
